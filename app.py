...
def submit_complaint():
    ...
    if request.method == 'POST':
        try:
            ...
            complaint_id = f"complaint_{timestamp}_{unique_id}"
            
            # Create a folder for this complaint
            complaint_folder = f"{complaint_id}/"
            
            # Save complaint data as JSON
            complaint_data = {
            ...
            # Upload complaint data to GCS
            bucket = storage_client.bucket(bucket_name)
            complaint_blob = bucket.blob(f"{complaint_folder}complaint.json")
            complaint_blob.upload_from_string(
                json.dumps(complaint_data),
                content_type='application/json'
            )
            
            # Process and upload image if provided via camera capture
            if photo_data and photo_data.startswith('data:image'):
                ...
                # Upload image to GCS
                image_blob = bucket.blob(f"{complaint_folder}image.jpg")
                image_blob.upload_from_string(
                    image_bytes,
                    content_type='image/jpeg'
                )
                complaint_data['image'] = f"gs://{bucket_name}/{complaint_folder}image.jpg"
            
            # Process and upload image if provided via file upload
            ...
            try:
                ...
                try:
                    ...
                    logging.error(f"Error publishing message to Pub/Sub: {str(e)}")
            except Exception as e:
                logging.error(f"Failed to initialize Pub/Sub publisher: {str(e)}")
            
            flash('Complaint submitted successfully! We will process it shortly.')
            return redirect(url_for('dashboard'))
            
        except Exception as e:
            logging.error(f"Error submitting complaint: {str(e)}")
            flash('An error occurred while submitting your complaint. Please try again.')
