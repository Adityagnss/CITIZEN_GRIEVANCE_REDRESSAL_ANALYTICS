...
def create_table_with_schema():
    ...
    try:
        ...
        # Create table with schema and clustering
        table = bigquery.Table(table_ref, schema=schema)
        
        # Add clustering by complaint_id to prevent duplicates
        table.clustering_fields = ["complaint_id"]
        
        table = bigquery_client.create_table(table)
        
        logging.info(f"Table {FULL_TABLE_ID} created successfully with updated schema and clustering")
        return True
    except Exception as e:
