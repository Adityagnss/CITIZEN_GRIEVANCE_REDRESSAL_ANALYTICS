...
def insert_into_bigquery(row, table_id):
    """Insert a row into BigQuery, ensuring schema compatibility."""
    try:
        # Get the table schema
        table = bigquery_client.get_table(table_id)
        schema_fields = [field.name for field in table.schema]
        
        # Filter row to match schema
        filtered_row = {k: v for k, v in row.items() if k in schema_fields}
        
        # Check for removed fields and log them
        removed_fields = set(row.keys()) - set(filtered_row.keys())
        if removed_fields:
            logging.warning(f"Removed fields not in schema: {removed_fields}")
        
        # Create a unique insert ID based on complaint_id to prevent duplicates
        insert_id = f"{row['complaint_id']}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
        
        # Insert row into BigQuery
        errors = bigquery_client.insert_rows_json(
            table_id, 
            [filtered_row],
            row_ids=[insert_id]  # Use insert_id to prevent duplicates
        )
